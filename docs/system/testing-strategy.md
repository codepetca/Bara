...
- Add browser smoke coverage when copy, open, or share behavior depends on real browser APIs or absolute URLs.
...
- Add Convex tests when token-based attendance collection or closed-session write rules change.
- Required when changing `/s/edit/[token]`, token lookups, or attendance toggling UX.

### QR Display And Shared Token Screens

- Use route-level tests when protected vs public access changes.
- Use Convex tests for token lookup behavior, invalid token handling, and closed-session display state.
- Use browser smoke tests when shared links are copied or opened from staff surfaces.
- Required when changing `/s/display/[token]`, public token queries, or the QR display contract.
...
- Use Playwright snapshots for screens that define the app's visual contract.
- Cover state families, not just canonical happy-path screens.
- High-value variants include:
  - open and closed attendance
  - success, warning, and failure student results
  - unopened vs active roster action states
  - invalid or unavailable shared-link screens
...
- Browser automation: required when jsdom is not enough for the behavior under change, especially for clipboard, copied-link usability, and public shared-link flows
...
For UI workflow changes, use this completion checklist:

- update the nearest behavior test
- add or update route or middleware coverage if protected vs public access changed
- add or update Convex coverage if token lookup or persistence rules changed
- add or update a visual fixture or snapshot if the visible contract changed
- add a browser smoke test if the flow depends on copy, open, share, clipboard, or real navigation
- inspect sibling screens and sibling states before considering the workflow complete
