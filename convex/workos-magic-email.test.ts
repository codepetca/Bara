...
async function signatureFor(payload: string, secret = webhookSecret) {
...
describe("WorkOS Magic Auth webhook", () => {
  ...
  it("accepts the opaque signing-secret length generated by WorkOS", async () => {
    const providerSecret = "test_workos_secret_length";
    expect(providerSecret).toHaveLength(25);
    vi.stubEnv("WORKOS_MAGIC_AUTH_BREVO_DELIVERY", "false");
    vi.stubEnv("WORKOS_MAGIC_AUTH_WEBHOOK_SECRET", providerSecret);
    const t = makeTest();
    const payload = eventPayload();
    const signature = await signatureFor(payload, providerSecret);
    const response = await postEvent(t, payload, signature);
  ...
  it("fails closed when the signing secret is missing", async () => {
    vi.stubEnv("WORKOS_MAGIC_AUTH_BREVO_DELIVERY", "false");
    vi.stubEnv("WORKOS_MAGIC_AUTH_WEBHOOK_SECRET", "");
    const t = makeTest();
    const response = await postEvent(t, eventPayload());
